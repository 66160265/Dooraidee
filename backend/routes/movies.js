...
// getDiscoverMovies already filters unreleased titles out server-side via
// primary_release_date.lte — TMDB's search endpoint has no such param, so
// search results get the same filter applied here instead.
function isReleased(releaseDate) {
    return !releaseDate || releaseDate <= new Date().toISOString().slice(0, 10);
}
...
router.get('/', async (req, res, next) => {
    try {
        ...
        const results = search ? data.results.filter((m) => isReleased(m.release_date)) : data.results;
        res.json({
            page: data.page,
